...
In this project, you are going to analyze one of Steve Jobs' famous commencement 
speech delivered in 2005 at Stanford University. You can download this speech here 
to test your program locally before submitting to zyBook. Specifically, you are going 
to process this file to identify all the co-occurring word-pairs and their frequencies. 
Two words are said to co-occur if they appear in the same sentence. We will refer to 
a pair of co-occurring words as word-pairs. The frequency of a word-pair is defined as 
the number of sentences that consist of this word-pair. You are required to include three 
...
You are going to include the following function prototypes in this header file 
(but please feel free to introduce other helper routines if you see necessary) :
    ...
    1. sentenceSplitter.
        ...
        What will be considered as sentence delimiters? Given a paragraph of multiple sentences, 
        the following punctuations will be used to split this paragraph into individual sentences
        - period: .
        - question mark: ?
        - period + double quotation mark: ."
        - question mark + double quotation mark: ?"
        - the newline character (i.e., the end of a paragraph in this project). 
        This takes care of the two paragraphs that end with the colon punctuation mark (:).

        Assume the input file contains the following three paragraphs (REMEBER TO RM NEW LINES):
        ...
        "We have an unexpected baby boy; do you want him?" They said: "Of course." My biological mother later 
        found out that my mother had never graduated from college and that my father had never graduated from high school. 
        She refused to sign the final adoption papers. She only relented a few months later when my parents promised that 
    ...
    2. identify unique word-pairs and calculate their frequencies.
        ...
        Given a list of sentences stored in the first argument sentences, this function identifies all the all the 
        unique word-pairs and each word-pair's frequency. The identified (word-pair, frequency)'s will be stored into 
        wordpariFreq_map, which is a map of (key, value) pairs. The key of this map a word-pair and the value is the 
        frequency of this word-pair.

        Note that:
        - Tokens are case insensitive. We will consider lower case in this project. Whitespaces will be the token delimiter.
        - The two words in a word-pair are different. For example, event though the first sentence above contains two the, 
          you are not going to construct a word pair <the, the>
        - Order does not matter between two words in a word-pair. For example, the word-pair <college, that> is the same as 
          <that, college>. You are recommended to arrange the two words in lexicographical order before inserting the pair 
          into the map.
    ...
    3. flip the map of to a multimap of to order all the word-pairs in ascending order of frequency
        ...
        For example, the first five word-pairs listed in the above step will be looking like the following, 
        where 3 is the key and the word-pair is the value:
        3: <about, connecting>
        3: <about, dots>
        3: <about, first>
        3: <about, is>
    ...
    4. output the most frequent and least frequent word-pairs to a file.
        ...
        You are going to simply use multimap's iterator and revserse_iterator to access the most frequent and 
        least frequent word-pairs. The output will be one word-pair per line in the format of <word1, word2>: frequency. 
        Assume the following word-pairs are the top 10 most frequent word-pairs. If topCnt is 10, the output file 
        will consist of the following 10 word-pairs together with their corresponding frequencies.
        <story, the>: 3
        <is, the>: 3
        <is, story>: 3
        <first, the>: 3
